...
class UPP_Multiple_Variant_Filter(BasePage):
    ...
    def test_hj_upp_variant_filter(self):
        try:
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + CLARITY + PRICE FILTERS: {result_count.upper()}")
        except:
            print(f"*****[HJ] FILTER IS NOT APPLIED IN THE VARIANT SECTION [UPP SKU: {self.SKU1}]..*****")
    ...
    def test_bb_upp_variant_filter(self):
        try:
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR FILTERS: {result_count.upper()}")
            self.click(self.clarity_vs2)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("BB_VARIANT_FILTER_SIZE_COLOUR_CLARITY")
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR + CLARITY FILTERS: {result_count.upper()}")
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR + CLARITY + CARAT FILTERS: {result_count.upper()}")
            min_price_slider = self.page.locator("a[data-target='priceMin']")
            min_price_slider.focus()
            min_price_slider.press("ArrowRight")
            self.timeout(8000)
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR + CLARITY + CARAT + PRICE FILTERS: {result_count.upper()}")
        except:
            print(f"*****[HJ] FILTER IS NOT APPLIED IN THE VARIANT SECTION [UPP SKU: {self.SKU2}]..*****")
    ...
    def test_bb_clear_all_filters(self):
        try:
            ...
            print(f"RECORDS AFTER CLEAR ALL FILTER: {result_count.upper()}")
        except:
            print("*****[BB] NOT ABLE TO PERFORM CLEAR ALL ACTION..*****")
    ...
    def test_hj_clear_all_filters(self):
        try:
            ...
            print("*****[HJ] NOT ABLE TO PERFORM CLEAR ALL ACTION..*****")
