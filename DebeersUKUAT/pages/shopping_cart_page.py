...
class Open_Shopping_Cart_Page(BasePage):
    ...
    def test_open_shopping_cart_page(self):
        try:
            ...
            self.timeout(2000)
            print("USER IS REDIRECTED TO THE CART PAGE..")
            self.screenshot.take_page_screenshot("CART_PAGE")
        except:
            print("*****USER IS NOT REDIRECTED TO THE CART PAGE..*****")

    def test_get_cart_products_count(self):
        try:
            count = self.get_text(self.added_product_count)
            count=int(count)
            print(f"TOTAL {count} PRODUCTS ARE AVAILABLE IN THE CART..")
            return count
        except:
            count=0
            return count
    ...
    def test_remove_product_from_cart(self):
        try:
            ...
            print("*****PRODUCT IS NOT REMOVED FROM THE CART PAGE..*****")
    ...
    def test_continue_to_checkout_from_cart(self):
        try:
            ...
            self.timeout(5000)
            print("USER IS REDIRECTED TO THE DELIVERY PAGE..")

        except:
            print("*****USER IS NOT REDIRECTED TO THE DELIVERY PAGE..*****")
