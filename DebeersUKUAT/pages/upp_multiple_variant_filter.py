...
class UPP_Multiple_Variant_Filter(BasePage):
    ...
    def test_hj_upp_variant_filter(self):
        try:
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + CLARITY + PRICE FILTERS: {result_count}")
        except:
    ...
    def test_bb_upp_variant_filter(self):
        try:
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR FILTERS: {result_count}")
            self.click(self.clarity_vs2)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("BB_VARIANT_FILTER_SIZE_COLOUR_CLARITY")
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR + CLARITY FILTERS: {result_count}")
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR + CLARITY + CARAT FILTERS: {result_count}")
            min_price_slider = self.page.locator("a[data-target='priceMin']")
            min_price_slider.focus()
            min_price_slider.press("ArrowRight")
            self.timeout(8000)
            ...
            result_count = self.inner_text(self.filter_result_count)
            print(f"RECORDS AFTER SIZE + COLOUR + CLARITY + CARAT + PRICE FILTERS: {result_count}")
        except:
    ...
    def test_bb_clear_all_filters(self):
        try:
            ...
            print(f"RECORDS AFTER CLEAR ALL FILTER: {result_count}")
        except:
