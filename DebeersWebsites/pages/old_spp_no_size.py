...
class OLD_SPP_No_Size(BasePage):
        ...
        def test_old_spp_no_size_with_engraving(self):
            try:
                ...
                logger.info(f"[OLD SPP WITHOUT SIZE AND WITH ENGRAVING] {self.SKU1} IS ADDED TO THE CART..")
                self.screenshot.take_page_screenshot("OLD_SPP_NO_SIZE_ADDED_WITH_ENGRAVING")
                self.click(self.minicart_close_icon)
            except:
                logger.error(f"*****[OLD SPP WITHOUT SIZE AND WITH ENGRAVING] {self.SKU1} IS NOT ADDED TO THE CART..*****")
        ...
        def test_old_spp_no_size_without_engraving(self):
            try:
                ...
                if self.is_visible(self.ADD_TO_BAG_CTA):
                    ...
                    logger.info(f"[OLD SPP WITHOUT SIZE AND WITHOUT ENGRAVING] {self.SKU1} IS ADDED TO THE CART..")
                    self.screenshot.take_page_screenshot("OLD_SPP_NO_SIZE_ADDED_WITHOUT_ENGRAVING")
                    self.click(self.minicart_close_icon)
                else:
                    logger.warning(f"*****[OLD SPP WITHOUT SIZE AND WITHOUT ENGRAVING] {self.SKU1} PRODUCT IS NOT BUYABLE..*****")
                    pass

            except:
                logger.error(f"*****[OLD SPP WITHOUT SIZE AND WITHOUT ENGRAVING] {self.SKU1} IS NOT ADDED TO THE CART..*****")
