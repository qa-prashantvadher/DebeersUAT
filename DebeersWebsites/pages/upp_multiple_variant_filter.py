...
class UPP_Multiple_Variant_Filter(BasePage):
    ...
    def test_hj_upp_variant_filter(self):
        try:
            ...
            logger.info(f"[HJ UPP MULTIPLE VARIANTS] RECORDS WITHOUT FILTER: {result_count.upper()}")
            self.select_value(self.ring_size_dropdown, self.ring_size_53)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("HJ_VARIANT_FILTER_SIZE")
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[HJ UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE FILTER: {result_count.upper()}")
            self.click(self.clarity_si1)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("HJ_VARIANT_FILTER_SIZE_CLARITY")
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[HJ UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE + CLARITY FILTERS: {result_count.upper()}")
            ...
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[HJ UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE + CLARITY + PRICE FILTERS: {result_count.upper()}")
        except:
            logger.error(f"*****[HJ UPP MULTIPLE VARIANTS] FILTER IS NOT APPLIED IN THE VARIANT SECTION [UPP SKU: {self.SKU1}]..*****")
    ...
    def test_bb_upp_variant_filter(self):
        try:
            ...
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS WITHOUT FILTER: {result_count.upper()}")
            self.select_value(self.ring_size_dropdown, self.ring_size_52)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("BB_VARIANT_FILTER_SIZE")
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE FILTER: {result_count.upper()}")
            self.click(self.colour_g)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("BB_VARIANT_FILTER_SIZE_COLOUR")
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE + COLOUR FILTERS: {result_count.upper()}")
            self.click(self.clarity_vs2)
            self.timeout(8000)
            self.screenshot.take_page_screenshot("BB_VARIANT_FILTER_SIZE_COLOUR_CLARITY")
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE + COLOUR + CLARITY FILTERS: {result_count.upper()}")
            ...
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE + COLOUR + CLARITY + CARAT FILTERS: {result_count.upper()}")
            min_price_slider = self.page.locator("a[data-target='priceMin']")
            min_price_slider.focus()
            min_price_slider.press("ArrowRight")
            self.timeout(8000)
            ...
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS AFTER SIZE + COLOUR + CLARITY + CARAT + PRICE FILTERS: {result_count.upper()}")
        except:
            logger.error(f"*****[BB UPP MULTIPLE VARIANTS] FILTER IS NOT APPLIED IN THE VARIANT SECTION [UPP SKU: {self.SKU2}]..*****")
    ...
    def test_bb_clear_all_filters(self):
        try:
            ...
            logger.info(f"[BB UPP MULTIPLE VARIANTS] RECORDS AFTER CLEAR ALL FILTER: {result_count.upper()}")
        except:
            logger.error("*****[BB UPP MULTIPLE VARIANTS] NOT ABLE TO PERFORM CLEAR ALL ACTION..*****")
    ...
    def test_hj_clear_all_filters(self):
        try:
            ...
            result_count = self.inner_text(self.filter_result_count)
            logger.info(f"[HJ UPP MULTIPLE VARIANTS] RECORDS AFTER CLEAR ALL FILTER: {result_count.upper()}")
        except:
            logger.error("*****[HJ UPP MULTIPLE VARIANTS] NOT ABLE TO PERFORM CLEAR ALL ACTION..*****")
