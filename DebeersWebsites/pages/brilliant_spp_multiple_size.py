...
class BB_SPP_Multiple_Size(BasePage):
        ...
        def test_bb_spp_multiple_size_with_engraving(self):
            try:
                ...
                self.timeout(2000)
                logger.info(f"[BB SPP MULTIPLE SIZE WITH ENGRAVING] {self.SKU1} IS ADDED TO THE CART..")
                self.screenshot.take_page_screenshot("BB_SPP_MULTIPLE_ADD_WITH_ENGRAVING")
                self.click(self.minicart_close_icon)
            except:
                logger.error(f"*****[BB SPP MULTIPLE SIZE WITH ENGRAVING] {self.SKU1} IS NOT ADDED TO THE CART..*****")
        ...
        def test_bb_spp_multiple_size_without_engraving(self):
            try:
                ...
                logger.info(f"[BB SPP MULTIPLE SIZE WITHOUT ENGRAVING] {self.SKU1} IS ADDED TO THE CART..")
                self.click(self.minicart_close_icon)
            except:
                logger.error(f"*****[BB SPP MULTIPLE SIZE WITHOUT ENGRAVING] {self.SKU1} IS NOT ADDED TO THE CART..*****")
